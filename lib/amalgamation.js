...
const NEEDS_MASTER_SINGLE = [2, 7, 0]; // versions <= this need single.pl and port.conf from master
...
function needsMasterSingle(resolved) {
    const v = parseVersion(resolved);
    return v && !versionGte(v, [NEEDS_MASTER_SINGLE[0], NEEDS_MASTER_SINGLE[1], NEEDS_MASTER_SINGLE[2] + 1]);
}

function patchFromMaster(resolved) {
    if (!needsMasterSingle(resolved)) return;

    // Overwrite single.pl with master version
    const singleContent = gitExec('show', 'master:single.pl');
    fs.writeFileSync(path.join(LEXBOR_DIR, 'single.pl'), singleContent);

    // Overwrite port.conf files from master for each port
    const portsDir = path.join(LEXBOR_DIR, 'source', 'lexbor', 'ports');
    if (!fs.existsSync(portsDir)) return;

    const ports = fs.readdirSync(portsDir).filter(entry => {
    ...
    for (const port of ports) {
        try {
            const confContent = gitExec('show', `master:source/lexbor/ports/${port}/port.conf`);
            fs.writeFileSync(path.join(portsDir, port, 'port.conf'), confContent);
        } catch (e) {
            // port.conf may not exist in master for this port, skip
        }
    }
}
...
function safeCheckout(ref) {
    try { gitExec('checkout', '--', '.'); } catch (e) {}
    try { gitExec('clean', '-fd', 'source/lexbor/ports'); } catch (e) {}
...
function buildCacheKey(resolvedVersion, modules, commitHash, port) {
    ...
    const versionPart = commitHash
        ? `master_${commitHash}`
        : resolvedVersion;
    const sortedPort = port && port !== 'all' ? port.split(',').sort().join(',') : '';
    const portPart = sortedPort ? `__port_${sortedPort}` : '';
...
function generate(version, modules, port) {
    ...
    return generationQueue.enqueue(() => {
        return new Promise((resolve, reject) => {
            ...
            // For old versions, use single.pl and port.conf from master
            patchFromMaster(resolved);
            ...
            const scriptPath = path.join(LEXBOR_DIR, 'single.pl');
            const portArgs = portValue !== 'all' ? [`--port=${portValue}`] : [];
            const args = [scriptPath, ...portArgs, ...sortedModules];
