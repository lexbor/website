...
function loadMetaCache() {
    try {
        if (fs.existsSync(META_CACHE_PATH)) {
            ...
            console.log(`[amalgamation] Loaded meta cache (${Object.keys(modulesCache).length} modules, ${Object.keys(depsCache).length} deps)`);
        }
    } catch (err) {
...
function cleanupMasterCache() {
    try {
        ...
        for (const cache of [modulesCache, depsCache]) {
